...
选择语言展开阅读。Choose a language section to expand.
...
This repository now only contains Claude Skills source files and packaged `.skill` artifacts. It no longer includes the old `/sv-*` pipeline slash commands, backend API docs, LangSmith prompt archives, or sample project state files.
